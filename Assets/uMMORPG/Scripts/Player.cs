...
public partial class Player : Entity {
    ...
    public override int healthMax {
        get {
            ...
            int equipmentBonus = (from slot in equipment
                                  where slot.amount > 0
                                  select ((EquipmentItemTemplate)slot.item.template).healthBonus).Sum();
    ...
    public override int manaMax {
        get {
            ...
            int equipmentBonus = (from slot in equipment
                                  where slot.amount > 0
                                  select ((EquipmentItemTemplate)slot.item.template).manaBonus).Sum();
    ...
    public override int damage {
        get {
            ...
            int equipmentBonus = (from slot in equipment
                                  where slot.amount > 0
                                  select ((EquipmentItemTemplate)slot.item.template).damageBonus).Sum();
    ...
    public override int defense {
        get {
            ...
            int equipmentBonus = (from slot in equipment
                                  where slot.amount > 0
                                  select ((EquipmentItemTemplate)slot.item.template).defenseBonus).Sum();
    ...
    public override float blockChance {
        get {
            ...
            float equipmentBonus = (from slot in equipment
                                    where slot.amount > 0
                                    select ((EquipmentItemTemplate)slot.item.template).blockChanceBonus).Sum();
    ...
    public override float criticalChance {
        get {
            ...
            float equipmentBonus = (from slot in equipment
                                    where slot.amount > 0
                                    select ((EquipmentItemTemplate)slot.item.template).criticalChanceBonus).Sum();
    ...
    [Header("PvP")]
    public BuffSkillTemplate offenderBuff;
    public BuffSkillTemplate murdererBuff;
...
//        Camera.main.GetComponent<CameraMMO>().target = transform;
        ...
        if (isServer)
            for (int i = 0; i < buffs.Count; ++i)
                if (buffs[i].BuffTimeRemaining() > 0)
                    buffs[i].template.SpawnEffect(this, this);
    ...
    public void CmdUseInventoryItem(int index) {
        ...
        if ((state == "IDLE" || state == "MOVING" || state == "CASTING") &&
            ...
            ItemTemplate item = inventory[index].item.template;
            if (item is UsableItemTemplate &&
                ((UsableItemTemplate)item).CanUse(this, index))
                ((UsableItemTemplate)item).Use(this, index);
        }
    }
    ...
    void RefreshLocation(int index) {
        ...
        if (info.requiredCategory != "" && info.location != null) {
            ...
            if (slot.amount > 0) {
                ...
                EquipmentItemTemplate template = (EquipmentItemTemplate)slot.item.template;
    ...
    public void CmdSwapInventoryEquip(int inventoryIndex, int equipmentIndex) {
        ...
        if (health > 0 &&
            ...
            ItemSlot slot = inventory[inventoryIndex];
            if (slot.amount == 0 ||
                slot.item.template is EquipmentItemTemplate &&
                ((EquipmentItemTemplate)slot.item.template).CanEquip(this, inventoryIndex, equipmentIndex)) {
                // swap them
    ...
    public override bool HasCastWeapon() {
        ...
        return equipment.FindIndex(slot => slot.amount > 0 &&
            ((EquipmentItemTemplate)slot.item.template).category.StartsWith("Weapon")
        ) != -1;
    }
    ...
    public void CmdAcceptQuest(int npcQuestIndex) {
        ...
        {
            QuestTemplate npcQuest = ((Npc)target).quests[npcQuestIndex];
    ...
    public void CmdCompleteQuest(int npcQuestIndex) {
        ...
        {
            Npc npc = (Npc)target;
            QuestTemplate npcQuest = npc.quests[npcQuestIndex];
            ...
            if (index != -1) {
                ...
                if (!quest.completed) {
                    ...
                    if (quest.IsFulfilled(gathered)) {
                        ...
                        if (quest.rewardItem == null || InventoryCanAdd(new Item(quest.rewardItem), 1)) {
    ...
    public void CmdCraft(int[] indices) {
        ...
        if ((state == "IDLE" || state == "MOVING") &&
            ...
            List<ItemTemplate> items = indices.Select(index => inventory[index].item.template).ToList();

            // find recipe
            RecipeTemplate recipe = RecipeTemplate.dict.Values.ToList().Find(r => r.CanCraftWith(items)); // good enough for now
    ...
    public void CmdNpcRevivePet(int index) {
        ...
        {
            ItemSlot slot = inventory[index];
            if (slot.amount > 0 && slot.item.template is PetItemTemplate) {
                // verify the pet status
                PetItemTemplate template = (PetItemTemplate)slot.item.template;
                if (slot.item.petHealth == 0 && template.petPrefab != null) {
                    // enough gold?
                    if (gold >= template.petPrefab.revivePrice) {
                        // pay for it, revive it
                        gold -= template.petPrefab.revivePrice;
                        slot.item.petHealth = template.petPrefab.healthMax;
                        inventory[index] = slot;
                    }
                }
            }
    ...
    void OnDragAndDrop_InventorySlot_NpcPetReviveSlot(int[] slotIndices) {
        // slotIndices[0] = slotFrom; slotIndices[1] = slotTo
        if (inventory[slotIndices[0]].item.template is PetItemTemplate)
